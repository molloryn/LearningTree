...
该隔离级别相当于没有加锁，允许脏读，属于隔离级别中最低的一种。举例来说，假设事务A和事务B同时进行，事务A在执行过程中会将某数据的值从1逐步增加到10，直至提交事务。此时，事务B能够读取到事务A在执行过程中的所有中间值（如1变成2，2变成3，依此类推），这些值可能并非最终提交的正确数据。

**脏读（`Dirty Read`）**：事务B在事务A未提交的情况下读取到的数据，实际上可能并不存在，因为这些中间数据可能被事务A回滚或修改。

---

###### 2. 读已提交（`Read Committed`）

每次查询数据时，数据库会对查询结果中的每一行加**共享锁**（或行级锁），而锁会在查询结束时被释放。

该隔离级别保证事务只能读取到已提交事务的数据，避免了脏读的发生。但它仍然无法避免幻读和不可重复读的问题。

**幻读（`Phantom Reads`）**：在事务A中，第一次读取的结果集可能与第二次读取的结果集不同，尤其在涉及到插入、删除等操作时，新的数据项可能被其他事务修改或添加。

**不可重复读（`Non-repeatable Reads`）**：在事务A中，第一次读取某条记录的字段值，第二次读取时可能会发现该字段值已发生变化。这种情况通常出现在读取单条数据时，可能由于其他事务修改了该数据。

---

###### 3. 可重复读（`Repeatable Read`）

此隔离级别会对读取的记录加上共享锁（读锁），并保持这个锁直到事务结束，其他事务不能修改这行数据，但仍然可以读取该数据。从而避免了不可重复读的问题。
然而，这个隔离级别**并不防止其他事务向表中插入新行**（即可能出现“幻读”问题）。

---

###### 4. 串行化（`Serializable`）

这是最严格的事务隔离级别，要求所有事务按顺序串行执行，不允许并发执行。它能够完全避免脏读、不可重复读和幻读等问题，确保事务之间没有任何并发干扰。
